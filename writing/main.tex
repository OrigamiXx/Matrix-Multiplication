...
bounds are known in general and also for specialized models of computation, like
bounded-depth arithmetic circuits, e.g., \cite{shp03,kam05}.
...
\end{lemma}
\noindent That said, there remains hope that the uniquely solvable puzzle
...
graph $H_P$ from $P$ and deciding whether it has a non-trivial 3D matching.  It well-known that 3D matching is \NP{}-complete, c.f., e.g., \cite{gj79}, and it can be efficiently reduced to
...
SAT solver (MapleCOMPSPS \cite{lgpc16}) to answer the reduced problem. To perform the
...
international SAT competition \cite{bhj17,lgpc16}.  This solver is conflict driven and uses a learning rate
...
they only form an independence system \cite{oxl06}.  In particular, the
...
One consequence is that na\"{i}ve greedy algorithms will not be effective
for finding maximum size strong USPs.  Furthermore, we do not
...
extend $P$ while maintaining that it is a strong USP. That said, we have had some success applying general purpose search
