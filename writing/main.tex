...
\begin{algorithmic}[1]
  ...
  \State{Construct $H_P$.}
  \Function{SearchHalf}{$Q,R,\ell,reverse?$}
  \If{$reverse?$}
    \If{$\ell = |P| - \lfloor|P|/2\rfloor$}
      \State{\Return{$true$ iff $(P-Q,P-R) \in T$}.}
    \EndIf
    \State{$\ell = \ell - 1$.}
  \Else
    \If{$\ell = \lfloor|P|/2\rfloor$}
  ...
  \For{$q \in P - Q$}
    \For{$r \in P - R$}
      \If{$(p_\ell, q, r) \in H_P$}
        \State{$result = result~\vee$ \textsc{SearchHalf}$(Q \cup \set{q}, R \cup \set{r}, \ell, reverse?)$}
      \EndIf
    \EndFor
  \EndFor
  \State{\Return{$result$.}}
  ...
  \State{\textsc{SearchHalf}$(\emptyset, \emptyset, 0, false)$.}
  \State{\Return{\textsc{SearchHalf}$(\emptyset, \emptyset, |P|+1, true)$}.}
...
\begin{algorithmic}[1]
  ...
  \For{$\ell'_Q = \ell_Q + 1$ to $|P|$}
    \For{$\ell'_R = \ell_R + 1$ to $|P|$}
  ...
  \State{\textsc{SearchHalf}$(1,\emptyset, 0, \emptyset, 0, 1, \lfloor |P| / 2 \rfloor + 1)$.}
  \State{\Return{\textsc{SearchHalf}$(|P|, \emptyset, 0, \emptyset, 0, -1, \lfloor |P|/2 \rfloor)$}.}
...
\begin{algorithmic}[1]
  ...
  \State{Let $T = \emptyset$.}
  \State{Construct $H_P$.}
  \Function{SearchForward}{$\ell, Q,\ell_Q, R,\ell_R$}
  \If{$\ell = \lfloor |P|/2 \rfloor + 1$}
    \State{Insert $(Q,R)$ into $T$.}
    \State{\Return{.}}
  \EndIf    
  \For{$\ell'_Q = \ell_Q + 1$ to $|P|$}
    \For{$\ell'_R = \ell_R + 1$ to $|P|$}
      \If{$(p_\ell, p_{\ell'_Q}, p_{\ell'_R}) \in H_P$}
        \State{\textsc{SearchForward}$(\ell + \delta, Q \cup \set{p_{\ell'_Q}}, \ell'_Q, R \cup \set{p_{\ell'_R}}, \ell'_R)$}
      \EndIf
    \EndFor
  \EndFor
  \EndFunction

  \Function{SearchReverse}{$\ell, Q,\ell_Q, R,\ell_R$}
  \If{$\ell = \lfloor |P|/2 \rfloor$}
    \State{\Return{$true$ iff $(P-Q,P-R) \in T$}.}
  \EndIf
  \State{$result = false$.}
  \For{$\ell'_Q = \ell_Q + 1$ to $|P|$}
    \For{$\ell'_R = \ell_R + 1$ to $|P|$}
      \If{$(p_\ell, p_{\ell'_Q}, p_{\ell'_R}) \in H_P$}
        \State{$result = result~\vee$ \textsc{SearchReverse}$(\ell + \delta, Q \cup \set{p_{\ell'_Q}}, \ell'_Q, R \cup \set{p_{\ell'_R}}, \ell'_R)$}
      \EndIf
    \EndFor
  \EndFor
  \State{\Return{$result$.}}
  ...
  \State{\textsc{SearchForward}$(1,\emptyset, 0, \emptyset, 0)$.}
  \State{\Return{\textsc{SearchReverse}$(|P|, \emptyset, 0, \emptyset, 0)$}.}
