...
%a puzzle $P$ that allows edges in $H_P$ to be included
%independently.
...
%% vertex can only be matched one a single edge and so have clauses
%% that exclude matching edges that overlap non-trivially (on one or
%% two coordinates):
%% \begin{equation}
...
%% one of diagonal edges not be used:
%% \begin{equation}
...
%% integer programming model. For every edge $e= \notin H_p$, a
...
al.~can achieve a better asymptotic time of $O(2^s\poly(s))$
...
\begin{algorithmic}[1]
  ...
  \For{$i = 1$ to $t$}
    ...
    \State{Let $m = 0.$} \Comment{Number of edges in matching.}
    \While{$m < s$} 
