...
puzzle $P$ into a conjunctive normal form (CNF) formula, $\Phi_P$, a depth-2 formula that is the AND of ORs of Boolean literals.  We input the CNF
formula $\Phi_P$ into a SAT solver to determine whether the formula is
satisfiable.  We construct $\phi_P$ so that \autoref{lem:verify-to-3dm} implies that $\Phi_P$
...
\begin{equation}
  \Phi_P^{\text{non-edge}} = \bigwedge_{e \in \overline{E}} \neg M_e.
\end{equation}
...
add clauses require that every vertex in $H_P$ is matched with some
hyperedge:
\begin{equation}
  \Phi_P^{\ge 1} = 
...
following way: $M_{u,v}^1 \wedge M_{u,w}^2 \Rightarrow M_{u,v,w}$.
