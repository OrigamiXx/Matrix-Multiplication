...
%% to use computers to locate large strong USP,
...
%It well-known that 3D
%matching is \NP{}-complete, c.f., e.g., \cite{gj79}.
Here we reduce
our 3D matching problem to the satisfability (SAT) problem on conjunctive normal form (CNF) formulas and
then use a state-of-the-art SAT solver to
resolve the reduced problem. To perform the reduction, we convert the
graph $H_P$
%that is obtained from the puzzle $P$
...
$\Psi_P$ is satisfiable iff $H_P$ has a non-trivial 3D
  matching.
%% We input the CNF formula $\Psi_P$ into a
%% SAT solver to determine whether the formula is satisfiable.
...
Third, we require that each vertex be matched with at most one hyperedge and so have clauses that exclude matching hyperedges that
...
Fourth, we exclude the trivial 3D matching by requiring that at least
one of the diagonal hyperedges not be used:
%\begin{equation}
...
To solve the satisfiability problem %to implement Line 4 of \autoref{alg:sat}
...
%Our goal is determine whether there is a
%non-trivial 3D matching $M$.
%that is a subset of the edges of $E$ and
%is vertex disjoint.
...
%  \label{eqn:cons2}
%\begin{aligned}
%% \forall w \in P,~~\sum_{u,v \in P} M_{u,v,w} = 1 \text{ and }
...
%% Gurobi is a commerical, closed-source solver, so we cannot
%% verify its assertion that the integer program is infeasible.  On
...
%% To verify a puzzle $P$ we run a battery of fast heuristics and
...
%% exact algorithms that were discussed in the previous section.
...
puzzles of size $s'$.
%There is also space overhead because the
%precomputed results are being cached.
%% We also note that for a puzzle
...
%% non-strong USP is low.
%% There appeared to be diminishing returns with
...
selecting a hyperedge of that vertex to put into the 3D matching.  If the
heuristic successfully contructs a 3D matching it returns NO indicating that
...
\begin{algorithmic}[1]
  \Require{An $(s,k)$-puzzle $P$.}
  \Ensure{YES, if $P$ is found to be strong USP, and NO otherwise.}
  %\Function{Verify}{$P$}
  \If{$s \le 2$} \Return{\Call{VerifyBruteForce}{$P$}.} \EndIf
  \If{$s \le 7$} \Return{\Call{VerifyDynamicProgramming}{$P$}.} \EndIf
  \If{$s \le 10$}
    \State{Return result if \Call{HeuristicDownwardClosed}{$P, 2$} is not MAYBE.}
    \State{\Return{\Call{VerifyDynamicProgramming}{$P$}.}}
    \EndIf
...
%% Altogether this gives an efficient algorithm to compute a
%% maximum set of independent vectors or, more generally, to complete a
%% maximum set with some given subset $S'$.
...
%, so that if $P_2$ is a strong
%USP and $P_1 \subseteq P_2$, then $P_1$ is a strong USP.
...
width-$k$ strong USP.
%Here constructive means that specific concrete strong
%USP are produced by our implementation that witness the bound.
...
\begin{figure}
  ...
  \end{multicols}
  \vspace{-3ex}
  \caption{Respresentative examples of maximum-size strong USP found for width $k = 1$ to $6$.}
  \label{fig:examples}
\end{figure}
...
%% and different algorithmic approaches we have applied to
%% this case where it quickly, on order of an hour of CPU time, gets to
%% $14$, and makes no further progress in the succeeding month.
We do
...
appearant that exhaustive search is not practical for $k \ge 6$
...
to best.  Notice that it matchs the dynamic programming algorithm
...
The second and third authors thank Union College for 
Summer Research Fellowships funding their work.
...
bidirectional dynamic programming strong USP verification algorithm that using the 3D
matching instance.

\begin{algorithm}[h]
  \caption{: Bidirectional Dynamic Programming}
  \label{alg:bi}
\begin{algorithmic}[1]
  \Require{An $(s,k)$-puzzle $P$.}
  \Ensure{YES, if $P$ is a strong USP and NO otherwise.}
  \Function{VerifyDynamicProgramming}{$P$}
  \State{Let $T = \emptyset$.}
  \State{Construct 3D matching instance $H_P$.}
...
\autoref{alg:sat} formalizes the reduction from strong USP verification to SAT.
...
\autoref{alg:ip} formalizes the reduction from strong USP verification to IP.

\begin{algorithm}[h]
  \caption{: Reduction to integer programming}
  \label{alg:ip}
\begin{algorithmic}[1]
  \Require{An $(s,k)$-puzzle $P$.}
  \Ensure{YES, if $P$ is a strong USP and NO otherwise.}
  \Function{VerifyIP}{$P$}
  \State{Construct 3D matching instance $H_P$.}
  \State{Create empty integer program $Q_P$ with Boolean variables $\condset{M_{u,v,w}}{u,v,w \in P}$.}
  \State{Add constraints of Section 3.5 to $Q_P$.}
  \If{\textsc{IsFeasible}$(Q_P)$}
  \State{\Return{NO}.}
  \Else
  \State{\Return{YES}.}
  \EndIf
...
\begin{algorithmic}[1]
  ...
  \For{$i = 1$ to $t$}
    ...
    \While{$m < s$} 
      ...
      \State{$m = m + 1.$}
      \EndWhile
    \If {$m > s$} \Return{NO}. \Comment{3D matching found so not SUSP, halt.} \EndIf
  \EndFor
  \State{\Return{MAYBE}.}
  \EndFunction
\end{algorithmic}
...
number of strong USP of width $k$ and
